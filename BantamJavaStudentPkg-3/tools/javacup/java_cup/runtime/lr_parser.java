...
/** This class implements a skeleton table driven LR bantam.parser.  In general,
 ...
 *  the bantam.parser uses a state machine (the "viable prefix recognition machine"
 *  built by the bantam.parser generator).  The current state of the machine is placed
 ...
 *  determine whether to shift or to reduce.  When the bantam.parser shifts, it
 *  changes to a new state by pushing a new Symbol (containing a new state) 
 *  onto the stack.  When the bantam.parser reduces, it pops the handle (right hand
 *  side of a production) off the stack.  This leaves the bantam.parser in the state
 ...
 *  indexes) to determine a new state to go to.  The bantam.parser then shifts to
 ...
 *  debug_parse() provide two versions of the main bantam.parser (the only difference
 ...
 *  input in order to execute all actions and re-synchronize the actual bantam.parser
 *  configuration.<p>
 *
 *  This is an abstract class which is normally filled out by a subclass
 *  generated by the JavaCup bantam.parser generator.  In addition to supplying
...
public abstract class lr_parser {
   ...
   *  be taken when the bantam.parser is in the given state (i.e., the given state
  ...
  /** Internal flag to indicate when bantam.parser should quit. */
  protected boolean _done_parsing = false;

  /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

  /** This method is called to indicate that the bantam.parser should quit.  This is
   ...
   *  at bantam.parser generation time.
   *
   * @param act_num   the internal index of the action to be performed.
   * @param parser    the bantam.parser object we are acting for.
  ...
  /** User code for initialization inside the bantam.parser.  Typically this
   *  initializes the scanner.  This is called before the bantam.parser requests
  ...
  /** Initialize the action object.  This is called before the bantam.parser does
   ...
   *  can be overriden by the generated bantam.parser using the code declared in
   ...
   *  done_parsing() has been called (typically because the bantam.parser has
  ...
  public Symbol parse() throws java.lang.Exception
    {
      ...
      /* set up direct reference to tables to drive the bantam.parser */
   ...
   *  debug_reduce() when shift and reduce moves are taken by the bantam.parser
  ...
  public Symbol debug_parse()
    throws java.lang.Exception
    {
      ...
      debug_message("# Initializing bantam.parser");
   ...
   *  bantam.parser's configuration, nor execute any actions. If we can  parse all
   ...
   *  normal bantam.parser to continue with the overall parse.
   *
   ...
   *  control back to the normal bantam.parser.  Consequently, this version of the
   *  bantam.parser performs all actions and modifies the real parse configuration.
      ...
      while(!_done_parsing)
	{
	  ...
	  if (act > 0)
	      ...
	      if (!advance_lookahead()) 
		{
		  ...
		  /* go back to normal bantam.parser */
		  return;
		}

 ...
 *   <dt> -bantam.parser name
 *   <dd> specify bantam.parser class name [default "bantam.parser"]
 *   <dt> -symbols name  
 ...
 *   <dt> -noscanner
 *   <dd> don't refer to java_cup.runtime.Scanner in the bantam.parser
...
public class Main {
  ...
  public static void main(String argv[]) 
      ...
      if (bantam.lexer.error_count == 0)
	{
	  ...
	  } else { // everything's okay, emit bantam.parser.
	      if (print_progress) System.err.println("Writing bantam.parser...");
	      open_files();
	      emit_parser();
	      did_output = true;
	  }
...
"    -bantam.parser name   specify bantam.parser class name [default \"bantam.parser\"]\n" +
...
"    -version       print the version information for CUP and exit\n"
  ...
  protected static void parse_args(String argv[])
      ...
      for (i=0; i<len; i++)
	{
	  ...
	  else if (argv[i].equals("-bantam.parser"))
	    {
	      /* must have an arg */
	      if (++i >= len || argv[i].startsWith("-") || 
				argv[i].endsWith(".cup")) 
		usage("-bantam.parser must have a name argument");
  ...
  /** Output file for the bantam.parser class. */
  ...
  protected static void parse_grammar_spec() throws java.lang.Exception
    {
      ...
      /* create a bantam.parser and parse with it */
	...
	bantam.lexer.emit_error("Internal error: Unexpected exception");
	throw e;
      }
    }
  ...
  /** Build the (internal) bantam.parser from the previously parsed specification.
   *  This includes:<ul>
  ...
  protected static void build_parser() throws internal_error
	...
	System.err.println("  Building state machine...");
      ...
      /* build the LR bantam.parser action and reduce-goto tables */
      if (opt_do_debug || print_progress) 
      ...
      if (emit.num_conflicts > expect_conflicts)
	{
	  System.err.println("*** More conflicts encountered than expected " +
			     "-- bantam.parser generation aborted");
	  bantam.lexer.error_count++; // indicate the problem.
  ...
  /** Call the emit routines necessary to write out the generated bantam.parser. */
  ...
  protected static void emit_summary(boolean output_produced)
    {
      ...
      System.err.println("  " + bantam.lexer.error_count + " error" +
	 plural(bantam.lexer.error_count) + " and " + bantam.lexer.warning_count +
	 " warning" + plural(bantam.lexer.warning_count));
